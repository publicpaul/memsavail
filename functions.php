<?php
  ...
  // create NONCE
  if ( ! function_exists( 'create nonce' ) ) {
    ...
    if ( ! function_exists ( 'do_messages') ) {
        function do_messages($insert=NULL)  {
            ...
            if ( $insert == true ) {
                $message .= '<p class="success">Data was inserted
                      successfully.</p>';
            else {
                $message .= '<p class="error">There was an error
                      with the submission.</p>';
            }
            $message .= '</div>';
  ...
  if ( ! function_exists ( 'process' ) ) {
      function process($post)  {
          ...
          //Validate Email
          $filter_email = filter_var($post['email']),
    ...
    //Filter input
          ...
          if ( ! empty ( $post['interests'] ) )  {
              foreach ( array_keys( $post['interests'] )as
                  $interest )  {
                  $interest[] = filter_var($interest,
                        FILTER_SANITIZE_STRING);
                  }
      // Sanitize 'interests', then serialize.
          ...
          $stmt = $mysql->prepare("
                INSERT INTO users
                    (name,email,frequency,interests,country,comment)
                VALUES(?,?,?,?,?,?)
              ");

          $stmt->bind_param("ssssss",
                $filter_post['name'], $filter_email,
                    $filter_post['frequency'],
                $filter_interests, $filter_post['country'],
                    $filter_post['comment']
          );
